...
注意客户端配置 `InsecureSkipVerify` 用来控制客户端是否证书和服务器主机名. 如果设置为 true, 则不会校验证书以及证书中的主机名和服务器主机名是否一致, 这可能导致中间人攻击. 因此该设置仅应在测试过程中置为 true.
